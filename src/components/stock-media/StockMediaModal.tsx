  ...
  const loadMore = async () => {
    ...
    try {
      ...
      if (res.success && res.hits && res.hits.length > 0) {
        ...
        setPage(nextPage);
        if (res.totalHits) setTotalHits(res.totalHits);
      }
    } catch (e) {
      console.error("Load more stock media error:", e);
    }
    ...
    <div className="fixed inset-0 z-[60] flex animate-in fade-in duration-200">
      ...
      <div className="relative z-10 flex flex-col w-full h-full bg-white dark:bg-[#0f1117]">
        ...
        <div className="flex-1 flex overflow-hidden">
          ...
          {/* Main results area */}
          <main ref={gridRef} className="flex-1 overflow-y-auto p-4 sm:p-5 flex flex-col justify-between">
            <div>
              {/* Loading */}
              {loading && (
              ...
              {/* ── Results Grid (Pixabay Style Header) ── */}
              {!loading && results.length > 0 && (
                <>
                  {/* Pixabay Format Heading: e.g. "10,136 Free images of Robotics" */}
                  <div className="flex items-center justify-between mb-5 border-b border-slate-100 dark:border-slate-800/80 pb-3">
                    <div>
                      <h2 className="text-lg sm:text-xl font-extrabold text-slate-900 dark:text-white tracking-tight">
                        {totalHits > 0 ? totalHits.toLocaleString() : results.length}{" "}
                        Free {mediaType === "video" ? "videos" : "images"}{" "}
                        <span className="capitalize font-semibold text-slate-500 dark:text-slate-400">
                          of {query || activeCategory || "Trending"}
                        </span>
                      </h2>
                      <p className="text-xs text-slate-400 mt-0.5">Showing {results.length} results</p>
                    </div>
                    <div className="flex items-center gap-1.5 text-xs text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/40 px-3 py-1.5 rounded-full font-bold border border-emerald-200 dark:border-emerald-800/50">
                      <Sparkles className="h-3.5 w-3.5" /> Free for commercial use
                    </div>
                  </div>
                  ...
                  {results.length < totalHits && (
                    <div className="flex justify-center my-8">
                      <button
                        ...
                        className="px-6 py-2.5 bg-emerald-500 hover:bg-emerald-600 text-white font-bold text-xs rounded-xl shadow-lg shadow-emerald-500/20 flex items-center gap-2 transition-all hover:scale-105 active:scale-95 disabled:opacity-50"
                      >
                        {loadingMore ? (
                          <>
                          ...
                          <>
                            Load More Media ({results.length} of {totalHits.toLocaleString()}) <ChevronRight className="h-4 w-4" />
                          </>
                        )}
                      </button>
                    </div>
        ...
        <footer className="flex-shrink-0 border-t border-slate-200 dark:border-slate-800 bg-white/95 dark:bg-[#0f1117]/95 backdrop-blur-md px-5 py-2.5 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <span className="text-[11px] text-slate-400 font-semibold">
              Showing {results.length} of {totalHits > 0 ? totalHits.toLocaleString() : results.length} results
            </span>
            <button
              onClick={onClose}
