                    ...
                    <div className="flex items-center gap-3">
                      {row.avatar ? (
                        <img
                          src={row.avatar}
                          alt=""
                          className="h-8 w-8 shrink-0 rounded-full object-cover border border-slate-200 dark:border-slate-800"
                        />
                      ) : (
                        <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-slate-100 text-xs font-semibold text-slate-700 dark:bg-slate-800 dark:text-slate-300">
                          {((row.name || row.email)[0] || "U").toUpperCase()}
                        </div>
                      )}
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-1.5">
                          <Link
                            href={`/adminshahid/users/${row.id}`}
                            className="truncate font-semibold text-slate-900 hover:text-primary dark:text-slate-100"
                            onClick={(e) => e.stopPropagation()}
                          >
                            {row.name || (row.email.includes("@") ? row.email.split("@")[0] : row.email)}
                          </Link>
                          {row.role === "ADMIN" && (
                            <span className="inline-flex items-center gap-0.5 rounded bg-primary/10 px-1.5 py-0.5 text-[10px] font-medium text-primary dark:bg-primary/20">
                              <ShieldCheck className="h-3 w-3" />
                              Admin
                            </span>
                          )}
                          {row.blockedAt && (
                            <span className="inline-flex items-center gap-0.5 rounded bg-rose-500/10 px-1.5 py-0.5 text-[10px] font-medium text-rose-600 dark:text-rose-400">
                        ...
                        <div className="flex items-center gap-1.5 text-[11px] text-muted-foreground">
                          ...
                          <span className="shrink-0 font-mono text-[10px] text-slate-400" title={row.id}>
